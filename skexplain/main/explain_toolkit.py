...
class ExplainToolkit(Attributes):
    ...
    def __init__(self, estimators=None, 
        ...
        self.global_obj = GlobalExplainer(estimators=self.estimators,
        ...
        self.local_obj = LocalExplainer(estimators=self.estimators,
                            estimator_names=self.estimator_names,
                            X=self.X,
                            y=self.y,
                            estimator_output=self.estimator_output,
    ...
    def __repr__(self):
        return 'ExplainToolkit(estimator=%s \n \
                                 estimator_names=%s \n \
                                 X=%s length:%d \n \
                                 y=%s length:%d \n \
                                 estimator_output=%s \n \
    ...
    def local_contributions(self, 
        ...
        if method not in ['shap', 'tree_interpreter']:
            raise ValueError("Invalid method! Method must be 'shap' or 'tree_interpreter'")
    ...
    def shap(self, shap_kwargs={'masker' : None, 'algorithm' : 'auto'}):
        ...
        include_ys=True
        if len(self.y) < 1:
            warnings.warn("""No y values were provided! 
    ...
    def plot_shap(self, 
        ...
        if isinstance(shap_values, xr.Dataset):
            ...
            if key not in shap_values.data_vars:
                raise KeyError (f"""{key} is not an available variable for this dataset! 

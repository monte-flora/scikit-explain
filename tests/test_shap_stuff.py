...
class TestExplainToolkit(unittest.TestCase):
    def setUp(self):
        ...
        X = np.stack([np.random.uniform(-1,1, size=n_X) for _ in range(n_vars)], axis=-1)
        ...
        self.results = self.explainer.shap(shap_kwargs={'masker' : 
...
class TestSHAP(TestExplainToolkit): 
    ...
    def test_compute_shap_with_no_masker(self):
        ...
        with self.assertRaises(Exception) as ex:
            results = explainer.shap(shap_kwargs={'masker' : None, 
                                     'algorithm' : 'permutation'})
    ...
    def test_plot_shap_dependence(self):
        ...
        # Plot no histdata. 
        self.explainer.plot_shap(features=features,

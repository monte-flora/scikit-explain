...
def interpert_RF(model, df_of_examples, feature_names=None):
    ...
    forecast_probabilities = model.predict_proba(df_of_examples) [:,1] *100.
...
def calculate_first_order_ALE(data, feature, quantiles=None, classification=True):
    ...
    for i in range(1, len(quantiles)):
        ...
        # get subset of data
        subset = train_set[(quantiles[i - 1] <= data[feature]) & (data[feature] < quantiles[i])]
        ...
        if len(subset) != 0:
            ...
            if (classification is True):
                ALE[i - 1] += (model.predict_proba(z_up) - model.predict_proba(z_low)).sum() / subset.shape[0]
            else:
                ALE[i - 1] += (model.predict(z_up) - model.predict(z_low)).sum() / subset.shape[0]
...
def calculate_first_order_ALE_categorical(predictor, train_set, feature, 
            features_classes, feature_encoder=None):

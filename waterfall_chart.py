...
def plot(ax, index, data, Title="", x_lab="", y_lab="",
    ...
    #Plot and label
    my_plot = ax.bar(range(0,len(trans.index)), blank, width=0.5, color='white')
    ax.bar(range(0,len(trans.index)), trans.amount, width=0.6,
    ...
    for index, row in trans.iterrows():
        ...
        if row['amount'] > 0:
            y += (pos_offset*2)
            ax.annotate(formatting.format(row['amount']),(loop,y),ha="center", color = 'g', fontsize=9)
        else:
            y -= (pos_offset*4)
            ax.annotate(formatting.format(row['amount']),(loop,y),ha="center", color = 'r', fontsize=9)
        loop+=1
    ...
    #Rotate the labels
    ax.set_xticks(range(0,len(trans))) #, trans.index)#, rotation=rotation_value)
    ax.set_xticklabels(trans.index, rotation=rotation_value)
